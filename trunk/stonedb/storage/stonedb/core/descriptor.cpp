...
bool Descriptor::IsSDBItemsEmpty() {
  if (IsType_OrTree())
    return tree->IsSDBItemsEmpty();
  else
    return (!attr.vc || attr.vc->GetSDBItems().empty()) && (!val1.vc || val1.vc->GetSDBItems().empty()) &&
           (!val2.vc || val2.vc->GetSDBItems().empty());
}
  ...
  if (left) {             // i.e., not a leaf
    ...
    if (desc.lop == common::LogicalOperator::O_AND) {
      if (left->desc.rv == common::RSValue::RS_NONE || right->desc.rv == common::RSValue::RS_NONE) {
        ...
        auto __diff =
            std::chrono::duration_cast<std::chrono::duration<float>>(std::chrono::high_resolution_clock::now() - __start);
        STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(), " __SUBQUERY DescTreeNode::EvaluatePack mit.NextPackrow()");
      ...
      if (left->desc.rv == common::RSValue::RS_ALL && right->desc.rv == common::RSValue::RS_ALL) {
        ...
        auto __diff = std::chrono::duration_cast<std::chrono::duration<float>>(
            std::chrono::high_resolution_clock::now() - __start);
        STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                    " __SUBQUERY DescTreeNode::EvaluatePack left mit.NextPackrow()");
      ...
      auto __diff =
          std::chrono::duration_cast<std::chrono::duration<float>>(std::chrono::high_resolution_clock::now() - __start);
      STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                  " __SUBQUERY DescTreeNode::EvaluatePack left right->EvaluatePack(mit) desc.lop == common::LogicalOperator::O_AND");        
      ...
      if (left->desc.rv == common::RSValue::RS_NONE) {
        ...
        auto __diff = std::chrono::duration_cast<std::chrono::duration<float>>(
            std::chrono::high_resolution_clock::now() - __start);
        STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                    " __SUBQUERY DescTreeNode::EvaluatePack left desc.lop != common::LogicalOperator::O_AND, left->desc.rv == common::RSValue::RS_NONE");
      ...
      auto __diff = std::chrono::duration_cast<std::chrono::duration<float>>(
            std::chrono::high_resolution_clock::now() - __start);
        STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                    " __SUBQUERY DescTreeNode::EvaluatePack left desc.lop != common::LogicalOperator::O_AND, right->desc.rv == common::RSValue::RS_NONE");
...

        {
          auto __diff = std::chrono::duration_cast<std::chrono::duration<float>>(
              std::chrono::high_resolution_clock::now() - __start);
          STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                      " __SUBQUERY DescTreeNode::EvaluatePack left desc.lop != common::LogicalOperator::O_AND, "
                      " else , left->EvaluatePack(mit)");
        }
...

        {
            ...
            STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                        " __SUBQUERY DescTreeNode::EvaluatePack left desc.lop != common::LogicalOperator::O_AND, "
                        " else , mit.RewindToPack(pack_start)");
          }
...

        {
            ...
            STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(),
                        " __SUBQUERY DescTreeNode::EvaluatePack left desc.lop != common::LogicalOperator::O_AND, "
                        " else , mit.OrPackFilter(pack_start, f.get());");
          }
    ...
    auto __diff =
        std::chrono::duration_cast<std::chrono::duration<float>>(std::chrono::high_resolution_clock::now() - __start);
    STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", __diff.count(), " __SUBQUERY DescTreeNode::EvaluatePack right");

  }

  auto diff =
      std::chrono::duration_cast<std::chrono::duration<float>>(std::chrono::high_resolution_clock::now() - start);
  STONEDB_LOG(LogCtl_Level::INFO, "Timer %f : %s", diff.count(), " __SUBQUERY DescTreeNode::EvaluatePack over");
}
...
bool DescTreeNode::IsSDBItemsEmpty() {
  if (left && right)
    return left->IsSDBItemsEmpty() && right->IsSDBItemsEmpty();
  else
    return desc.IsSDBItemsEmpty();
}
